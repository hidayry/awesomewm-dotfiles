...
open_pictures:connect_signal(
  'invoked',
  function()
    awful.spawn('nemo ' .. 'Pictures', false)
  end
)
...
local screenshot_name = "screenshot_" .. os.date("%Y-%m-%d_%H-%M-%S") .. ".png"
    awful.spawn.easy_async_with_shell("sleep 0.3 && maim ~/Pictures/" .. screenshot_name,
      function()
        naughty.notify
        (
          {
...
timer_button:connect_signal("button::release", function(_, _, _, button)
  if button == 1 then
    ss_tool.visible = false
local screenshot_name = "screenshot_" .. os.date("%Y-%m-%d_%H-%M-%S") .. ".png"
    awful.spawn.easy_async_with_shell("sleep 0.3 && maim -d " .. delay_time .. " ~/Pictures/" .. screenshot_name,
      function()
        naughty.notify
        (
          {
...
selection:connect_signal("button::release", function(_, _, _, button)
  if button == 1 then
    ss_tool.visible = false
local screenshot_name = "screenshot_" .. os.date("%Y-%m-%d_%H-%M-%S") .. ".png"
    awful.spawn.easy_async_with_shell("sleep 0.3 && maim -s ~/Pictures/" .. screenshot_name, function()
        naughty.notify
        (
          {
