...
return function(screen, pinned, size, offset, modules_spacing, active_color,
    ...
    local pin_app_creator = function(app_command, app_name)
        ...
        local app_icon = icon_handler("Tela-circle-manjaro-dark", nil,
        ...
        w:connect_signal("mouse::enter", function() w.opacity = 0.7 end)
        w:connect_signal("mouse::leave", function() w.opacity = 1 end)

        w:connect_signal("button::press", function()
            awful.spawn.with_shell(app_command, false)
            w.opacity = 0.5
        end)
        w:connect_signal("button::release", function() w.opacity = 0.7 end)
    ...
    dock:connect_signal("mouse::enter", function() hidetimeout:stop() end)

    client.connect_signal("manage", function() when_no_apps_open(screen) end)

    client.connect_signal("unmanage", function() when_no_apps_open(screen) end)
    -- Eof dock visibility
    ----------------------------------------------------------------------------
